...
void OS_Callback(void const * argument);
...
{
  /* USER CODE BEGIN 5 */
	osTimerStart(PeriodicTimerHandle, 100);
  /* Infinite loop */
  for(;;)
  {
	uint8_t mssg[] = "LEVI from WALL_MARIYA (UART)\n";
	HAL_UART_Transmit(&huart2, mssg, sizeof(mssg), 100);
    osDelay(1000);
  }
  /* USER CODE END 5 */
}
...
void LED(void const * argument)
{
  ...
  for(;;)
  {
	  if(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0)){
		  ...
		  osTimerStart(OnceTimerHandle, 2000);
	  }
...
void PTCallback01(void const * argument)
{
  /* USER CODE BEGIN PTCallback01 */
	uint8_t mmsg[] = "EREN YEAGER from WALL_ROSE (Periodic)";
		  HAL_UART_Transmit(&huart2, mmsg, sizeof(mmsg), 100);
  /* USER CODE END PTCallback01 */
}

/* OS_Callback function */
void OS_Callback(void const * argument)
{
  /* USER CODE BEGIN OS_Callback */
	HAL_GPIO_WritePin(GPIOD, GPIO_PIN_14, GPIO_PIN_0);
  /* USER CODE END OS_Callback */
}
